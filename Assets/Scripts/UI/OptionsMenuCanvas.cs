...
public class OptionsMenuCanvas : MonoBehaviour
{
    ...
    public static OptionsMenuCanvas Instance
    {
        get
        {
            if (_instance == null)
            {
                _instance = FindAnyObjectByType<OptionsMenuCanvas>() ??
                            new GameObject("OptionsMenuCanvas").AddComponent<OptionsMenuCanvas>();
            }
    ...
    private GameAudioManager GameAudio => GameAudioManager.Instance;
    ...
    void Setup()
    {
        ...
        {
            currentResolution = new ResItem
            { width = defaultWidth, height = defaultHeight }; // Set to default if not found
        }
    ...
    {
        Container.SetActive(false);
        GameAudio.VolumeSounds = sfxVolumeSlider.value;
        GameAudio.VolumeMusic = musicVolumeSlider.value;
    }

    public void ShowMenu()
    {
        Container.SetActive(true);
        UpdateSoundMusicView();
        UpdateResolutionOptions();
    }

    private void UpdateSoundMusicView()
    {
        musicVolumeSlider.value = GameAudio.VolumeMusic;

        sfxVolumeSlider.value = GameAudio.VolumeSounds;
    }
    ...
    public void UpdateResolutionOptions()
    {
        ...
        int currentResolutionIndex =
            resolutions.FindIndex(res => res.width == Screen.width && res.height == Screen.height);
