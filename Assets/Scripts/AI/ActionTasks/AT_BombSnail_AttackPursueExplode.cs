...
public class AT_BombSnail_AttackPursueExplode : ActionTask
{
    private float m_pursueSpeed = 3f;
    private float m_detonationTimer = 3f;
    ...
    protected override UpdateStatus OnUpdate(float deltaTime)
    {
        ...
        var distance = (self.position - target.position).magnitude;

        Vector3 dir = (self.position - target.position).normalized;

        navMeshAgent.SetDestination(target.position + dir * 0.25f);
        navMeshAgent.speed = m_pursueSpeed;
        

        // Detonation timer
        m_detonationTimer -= Time.deltaTime;
        if (m_detonationTimer <= 0 || distance < 1f)
        {
